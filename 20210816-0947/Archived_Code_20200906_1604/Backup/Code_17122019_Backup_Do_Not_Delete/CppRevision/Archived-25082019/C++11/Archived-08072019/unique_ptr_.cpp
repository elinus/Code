...
public:
  Test(int value) : value(value) { std::cout << "Constructor" << std::endl; }
...
int main(int argc, char *argv[]) {
  ...
  // error: conversion from ‘Test*’ to non-scalar type ‘std::unique_ptr<Test>’
  // requested
  ...
  // std::unique_ptr<_Tp, _Dp>&) [with _Tp = Test; _Dp =
  // std::default_delete<Test>]’ std::unique_ptr<Test> p4(p3);
