...
struct sortedStruct {
  ...
  bool operator<(const sortedStruct &rhs) const { return (val < rhs.val); }
};
...
public:
  ...
  bool operator<(const sortedClass &sc) const { return val < sc.getVal(); }
...
int main(int argc, char *argv[]) {
  ...
  random_shuffle(vecSortClas.begin(), vecSortClas.end());

  random_shuffle(vecSortStru.begin(), vecSortStru.end());
  ...
  for (vector<sortedClass>::iterator it = vecSortClas.begin();
       it != vecSortClas.end(); it++) {
    cout << "sortedClass index " << distance(vecSortClas.begin(), it)
         << " has value " << it->getVal() << "\n";
  }
  ...
  for (vector<sortedStruct>::iterator it = vecSortStru.begin();
       it != vecSortStru.end(); it++) {
    cout << "sortedClass index " << distance(vecSortStru.begin(), it)
         << " has value " << it->val << "\n";
  }
