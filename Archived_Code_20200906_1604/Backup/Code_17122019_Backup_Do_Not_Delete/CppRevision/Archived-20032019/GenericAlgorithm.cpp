...
InputIterator findfirstinstance(InputIterator iterBegin, InputIterator iterEnd,
                                const valueType &findEle) {
...
OutputIterator
mergeSortedContainers(InputIterator lhsBegin, InputIterator lhsEnd,
...
void randomise(RandomAccessIterator iterBegin, RandomAccessIterator iterEnd) {
  while (iterBegin < iterEnd) {
    iter_swap(iterBegin,
              iterBegin +
                  rand() % (iterEnd -
                            iterBegin)); // randomInteger(iterEnd - iterBegin));
    ++iterBegin;
  }
}
...
int main(int argc, char *argv[]) {
  ...
  cout << "Results of calling mergeSortedContainers with firstList and "
          "secondList are."
       << "\n";
  mergeSortedContainers(firstList.begin(), firstList.end(), secondList.begin(),
                        secondList.end(), resultsList.begin());

  for (std::vector<int>::iterator it = resultsList.begin();
       it != resultsList.end(); it++) {
    cout << *it << " ";
  }
  cout << "\n\n";
  ...
  for (std::forward_list<int>::iterator it = firstList.begin();
       it != firstList.end(); it++) {
    cout << *it << " ";
  }
  cout << "\n\n";

  cout << "Results of calling findandreplace on secondList with values 76 and "
          "1006"
       << "\n";
  findandreplace(secondList.begin(), secondList.end(), 76, 1006);
  for (std::forward_list<int>::iterator it = secondList.begin();
