...
static jint setVal(JNIEnv *env, jclass clazz, jint fd, jint pos, jint num) {
    for(int i = 0; i < num; i++) {
        if(maps[i].fd == fd) {
...
static jint getVal(JNIEnv *env, jclass clazz, jint fd, jint pos) {
    for(int i = 0; i < num; i++) {
        if(maps[i].fd == fd) {
            if(pos < (maps[i].size/ sizeof(int))) {
...
static JNINativeMethod method_table[] = {
        { "setVal", "(III)I", (void *) setVal },
        { "getVal", "(II)I", (void *) getVal },
